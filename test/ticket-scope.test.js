...
function addWorkShift(db, staffId, workDate = '2026-08-13', start = '00:00', end = '23:59') {
  db.run(`INSERT OR REPLACE INTO shift_assignments
    (staff_id, work_date, assignment_type, start_at, end_at)
    VALUES (?, ?, 'work', ?, ?)`, [
    staffId, workDate, `${workDate}T${start}:00+08:00`, `${workDate}T${end}:00+08:00`,
  ]);
}
...
test('创建拒绝未知工单类型，主管创建处理中工单必须稳定派单', async (t) => {
  ...
  const assigned = await request(server, '/api/tickets', SUPERVISOR, {
    ...
    body: JSON.stringify({
      ...
      created: '2026-08-13T09:00:00+08:00', estimated_hours: 2,
    }),
  });
...
test('主管只能在有效工作班次内创建处理中工单', async (t) => {
  const cases = [
    { type: null, code: 'ASSIGNEE_NOT_SCHEDULED' },
    { type: 'rest', code: 'ASSIGNEE_RESTING' },
    { type: 'leave', code: 'ASSIGNEE_ON_LEAVE' },
    { type: 'work', start: '10:00', end: '18:00', code: 'ASSIGNMENT_OUTSIDE_SHIFT' },
    { type: 'work', start: '08:00', end: '10:00', code: 'ASSIGNMENT_OUTSIDE_SHIFT' },
  ];
  for (const [index, entry] of cases.entries()) {
    const db = await fixture();
    const server = await startHttpServer(db);
    t.after(() => server.close());
    if (entry.type === 'work') addWorkShift(db, 4, '2026-08-13', entry.start, entry.end);
    else if (entry.type) db.run(`INSERT INTO shift_assignments
      (staff_id, work_date, assignment_type, leave_type)
      VALUES (4, '2026-08-13', ?, ?)`, [entry.type, entry.type === 'leave' ? '事假' : null]);
    const result = await request(server, '/api/tickets', SUPERVISOR, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        id: `SHIFT-CHECK-${index}`, type: 'repair', status: 'doing', worker: '管家',
        community_id: 'c1', loc: '班次校验', created: '2026-08-13T09:00:00+08:00',
        estimated_hours: 2,
      }),
    });
    assert.equal(result.response.status, 409, entry.code);
    assert.equal(result.body.code, entry.code);
  }
});

test('主管创建重叠工单时返回冲突工单编号', async (t) => {
  const db = await fixture();
  addWorkShift(db, 4, '2026-08-13', '08:00', '18:00');
  db.run(`INSERT INTO tickets
    (id, type, community_id, status, worker, created, estimated_hours,
     assignee_user_id, assignee_staff_profile_id, assigned_at)
    VALUES ('KEEPER-BUSY', 'repair', 'c1', 'doing', '管家',
      '2026-08-13T09:00:00+08:00', 2, 4, 4, '2026-08-13T09:00:00+08:00')`);
  const server = await startHttpServer(db);
  t.after(() => server.close());
  const result = await request(server, '/api/tickets', SUPERVISOR, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      id: 'KEEPER-OVERLAP', type: 'help', status: 'doing', worker: '管家',
      community_id: 'c1', loc: '冲突派单', created: '2026-08-13T10:00:00+08:00',
      estimated_hours: 1,
    }),
  });
  assert.equal(result.response.status, 409);
  assert.equal(result.body.code, 'ASSIGNMENT_TIME_CONFLICT');
  assert.deepEqual(result.body.conflicting_ticket_ids, ['KEEPER-BUSY']);
});

test('修改已派工单预计时长仍必须完整处于原班次内', async (t) => {
  const db = await fixture();
  const now = new Date();
  const workDate = currentShanghaiDate();
  const shiftStart = new Date(now.getTime() - 3600000).toISOString();
  const shiftEnd = new Date(now.getTime() + 2 * 3600000).toISOString();
  db.run(`INSERT INTO shift_assignments
    (staff_id, work_date, assignment_type, start_at, end_at)
    VALUES (4, ?, 'work', ?, ?)`, [workDate, shiftStart, shiftEnd]);
  db.run(`UPDATE tickets SET assignee_staff_profile_id = 4, assigned_at = ?,
    estimated_hours = 1 WHERE id = 'K-R'`, [now.toISOString()]);
  const server = await startHttpServer(db);
  t.after(() => server.close());

  const result = await patchTicket(server, 'K-R', SUPERVISOR, { estimated_hours: 3 });
  assert.equal(result.response.status, 409);
  assert.equal(result.body.code, 'ASSIGNMENT_OUTSIDE_SHIFT');
});
...
test('只有主管可稳定派单并最终完成，重名或非在职直属人员拒绝派单', async (t) => {
  const db = await fixture();
  addWorkShift(db, 4, currentShanghaiDate());
  const server = await startHttpServer(db);
  t.after(() => server.close());
