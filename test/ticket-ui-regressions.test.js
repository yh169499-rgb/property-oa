...
test('工单抽屉移动端将现场材料与操作区独立布局', () => {
  assert.match(source, /class="photos"/);
  assert.match(source, /class="actions"/);
  assert.match(styles, /\.drawer-photo-section/);
  assert.match(styles, /\.drawer-actions/);
  assert.match(styles, /@media \(max-width: 480px\)[\s\S]*\.drawer-actions[\s\S]*grid-template-columns:\s*1fr/);
});
