...
test('每名普通测试人员均有完整已完成样本和当前工单', () => {
  ...
  const missingProfileIdentity = one(db, `SELECT COUNT(*) AS total FROM tickets t
    JOIN users u ON u.id = t.assignee_user_id
    JOIN staff_profiles sp ON sp.user_id = u.id
    WHERE t.id LIKE 'MOCK-E2E-%' AND t.assignee_user_id IS NOT NULL
      AND t.assignee_staff_profile_id IS NOT sp.id`);
  assert.equal(Number(missingProfileIdentity.total), 0);
});
...
test('今日正常工单不冲突，前一日保留明确的两工单重叠样本', () => {
  const { migrateRetainedTestData } = require('../services/retained-test-data');
  ...
  const todayCalendar = buildDayCalendar(db, {
    date: '2026-08-12', staffId: zhang.id, communityId: 'mock-e2e-community', viewerUserId: 1,
  });
  assert.deepEqual(todayCalendar.events.map(event => event.ticketId), ['MOCK-E2E-02-CURRENT']);
  assert.deepEqual(todayCalendar.conflicts, []);
  assert.equal(todayCalendar.events[0].startAt, '2026-08-12T01:00:00.000Z');
  assert.equal(todayCalendar.events[0].endAt, '2026-08-12T03:00:00.000Z');

  const conflictCalendar = buildDayCalendar(db, {
    date: '2026-08-11', staffId: zhang.id, communityId: 'mock-e2e-community', viewerUserId: 1,
  });
  const conflict = conflictCalendar.conflicts.find(item => item.ticketIds
    .includes('MOCK-E2E-02-CONFLICT'));
  assert.ok(conflict);
  assert.deepEqual(conflict.ticketIds, ['MOCK-E2E-02-CONFLICT', 'MOCK-E2E-02-CONFLICT-PEER']);
  const conflictEvents = conflictCalendar.events.filter(event => conflict.ticketIds.includes(event.ticketId));
  assert.deepEqual(conflictEvents.map(event => [event.ticketId, event.startAt, event.endAt]), [
    ['MOCK-E2E-02-CONFLICT', '2026-08-11T06:00:00.000Z', '2026-08-11T07:00:00.000Z'],
    ['MOCK-E2E-02-CONFLICT-PEER', '2026-08-11T06:30:00.000Z', '2026-08-11T07:30:00.000Z'],
  ]);
  ...
  assert.equal(liCalendar.events.some(event => event.ticketId === 'MOCK-E2E-03-CURRENT'), false);
  const liNextDay = buildDayCalendar(db, {
    date: '2026-08-13', staffId: li.id, communityId: 'default', viewerUserId: 1,
  });
  assert.equal(liNextDay.people[0].shift.assignmentType, 'work');
  assert.deepEqual(liNextDay.events.map(event => event.ticketId), ['MOCK-E2E-03-CURRENT']);
});
