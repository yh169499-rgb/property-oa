...
test('验证器确认账号、组织、工单和绩效样本完整', async () => {
  ...
  assert.equal(result.accounts.active, 5);
  assert.equal(result.accounts.loginVerified, 5);
  assert.equal(result.organization.managedBySupervisor, 4);
  assert.equal(result.organization.activeProfiles, 5);
  ...
  assert.equal(result.calendar.todayTicketConflicts, 0);
  assert.equal(result.calendar.currentOutsideWorkShift, 0);
  assert.equal(result.reporting.activeStaff, 5);
  assert.equal(result.reporting.scoredStaff, 4);
