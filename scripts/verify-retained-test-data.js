...
function inspectDatabase(db, password) {
  ...
  const conflictRows = rows(db, `SELECT t.id, sp.id AS staff_id, t.assigned_at, t.created, t.estimated_hours
    ...
    WHERE t.id IN ('MOCK-E2E-02-CONFLICT', 'MOCK-E2E-02-CONFLICT-PEER')`);
  ...
  const todayRows = rows(db, `SELECT t.id, sp.id AS staff_id, t.assigned_at, t.created, t.estimated_hours
    FROM tickets t JOIN staff_profiles sp ON sp.user_id = t.assignee_user_id
    WHERE t.id = 'MOCK-E2E-02-CURRENT'`);
  const todayTicketConflicts = detectCalendarConflicts(todayRows.map(ticket => {
    const startAt = ticket.assigned_at || ticket.created;
    return {
      ticketId: ticket.id,
      staffId: Number(ticket.staff_id),
  ...
  const currentRows = rows(db, `SELECT t.id, sp.id AS staff_id, t.assigned_at, t.created, t.estimated_hours
    FROM tickets t JOIN staff_profiles sp ON sp.user_id = t.assignee_user_id
    WHERE t.id LIKE 'MOCK-E2E-%-CURRENT'`);
  const workShifts = rows(db, `SELECT staff_id, start_at, end_at
    FROM shift_assignments WHERE assignment_type = 'work'`);
  const currentOutsideWorkShift = currentRows.filter(ticket => {
    const ticketStart = Date.parse(ticket.assigned_at || ticket.created);
    const ticketEnd = ticketStart + Number(ticket.estimated_hours || 1) * 3600000;
    return !workShifts.some(shift => Number(shift.staff_id) === Number(ticket.staff_id)
      && Date.parse(shift.start_at) <= ticketStart && Date.parse(shift.end_at) >= ticketEnd);
  }).length;
  const calendar = {
    ...
    ticketConflicts,
    todayTicketConflicts,
    currentOutsideWorkShift,
  };
  if (calendar.templates !== 2 || calendar.assignments < 12 || calendar.leave < 1
      ...
      || calendar.todayTicketConflicts !== 0 || calendar.currentOutsideWorkShift !== 0) {
