...
const { disableAccount, activeGlobalManagerCount } = require('../services/account-lifecycle');
...
router.post('/pending-registrations/:id/approve', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    run('INSERT INTO users (phone, password, name, role) VALUES (?, ?, ?, ?)', [reg.phone, reg.password, reg.name, ['keeper'].includes(String(reg.role || '').toLowerCase()) ? 'keeper' : 'worker']);
  } catch(e) {
...
router.post('/users', requireAuth, requireAdmin, async (req, res) => {
  ...
  if (!phone || !password || !name) return res.status(400).json({ error: '手机号、密码、姓名必填' });
  const safeRole = String(role || '').toLowerCase() === 'keeper' ? 'keeper' : 'worker';
  try {
    const hash = await bcrypt.hash(password, 10);
    run('INSERT INTO users (phone, password, name, role) VALUES (?, ?, ?, ?)', [phone, hash, name, safeRole]);
    await saveDB();
...
router.get('/users', requireAuth, requireAdmin, (req, res) => {
  const users = usersHaveStatusColumn()
    ? queryAll('SELECT id, phone, name, role, COALESCE(status, \'active\') AS status FROM users ORDER BY id')
...
router.delete('/users/:id', requireAuth, requireAdmin, async (req, res) => {
  ...
  if (['admin', 'manager', 'supervisor', '主管', '经理'].includes(String(target.role || '').trim().toLowerCase())
      && activeGlobalManagerCount(userId) === 0) {
    return res.status(409).json({ error: '系统至少需要保留一名有效主管' });
  }
  try {
    disableAccount(userId);
    await saveDB();
    res.json({ success: true, disabled: true, message: '账号已停用，登录权限已立即撤销' });
  } catch (error) {
    res.status(500).json({ error: '停用账号失败，请稍后重试' });
  }
});
