...
function upsertMockTicket(db, ticket) {
  ...
  const assigneeStaffProfileId = ticket.assignee_user_id == null
    ? null
    : one(db, 'SELECT id FROM staff_profiles WHERE user_id = ?', [ticket.assignee_user_id])?.id || null;
  const columns = [
    ...
    'performance_rule_version_id', 'assignee_staff_profile_id',
  ];
  const values = columns.map(column => column === 'assignee_staff_profile_id'
    ? assigneeStaffProfileId : ticket[column]);
  if (existing) {
...
function seedMockTickets(db, users, profiles, now) {
  ...
  const conflictDate = offsetDate(now, -1);
  const conflictAssignedAt = isoAtShanghai(conflictDate, 14, 0);
  ...
  upsertMockTicket(db, {
    ...
    worker: conflictAccount.name, message: '与另一张冲突演示工单时间重叠',
    created: new Date(Date.parse(conflictAssignedAt) - 10 * 60000).toISOString(),
    finished: '', reject_reason: '', estimated_hours: 1,
  ...
  const conflictPeerAssignedAt = isoAtShanghai(conflictDate, 14, 30);
  const conflictPeerId = 'MOCK-E2E-02-CONFLICT-PEER';
  upsertMockTicket(db, {
    id: conflictPeerId, type: 'repair', cat: '公共设施', desc: '全流程模拟第二张重叠工单',
    loc: '测试小区-设备间', priority: 'normal', status: 'doing',
    worker: conflictAccount.name, message: '用于演示两张工单互相重叠',
    created: new Date(Date.parse(conflictPeerAssignedAt) - 10 * 60000).toISOString(),
    finished: '', reject_reason: '', estimated_hours: 1,
    community_id: MOCK_COMMUNITY.id, repeat_key: '', repeat_of: '', repeat_count: 1,
    is_recurring: 0, recurrence_note: '', feedback_count: 1,
    metadata: JSON.stringify({ mock: true, scenario: 'calendar-conflict-peer' }),
    assignee_user_id: Number(conflictUser.id), assigned_at: conflictPeerAssignedAt,
    performance_rule_version_id: Number(rule.id),
  });
  [
    { key: `${conflictPeerId}:assign`, action: 'assign', actorUserId: Number(supervisor.id), actorStaffId: Number(supervisorProfile.id) },
    { key: `${conflictPeerId}:accept`, action: 'accept', actorUserId: Number(conflictUser.id), actorStaffId: Number(conflictProfile.id) },
  ].forEach((event, index) => upsertMockActivity(db, {
    ticketId: conflictPeerId, key: event.key, scenario: 'calendar-conflict-peer',
    actorUserId: event.actorUserId, actorStaffId: event.actorStaffId,
    action: event.action,
    createdAt: new Date(Date.parse(conflictPeerAssignedAt) + index * 5 * 60000).toISOString(),
  }));
